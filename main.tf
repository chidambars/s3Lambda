resource "aws_lambda_function" "myfunc" {
  ...
  function_name    = "mylambdafunction_chidambar"
...
resource "aws_iam_role" "lambdarole" {
  name               = "basiclambdarole-chidambar"
...
resource "aws_s3_bucket" "s3bucket" {
  bucket = "s3lambdatriggerexamplechidu"
}

resource "aws_cloudwatch_log_group" "cwlogs" {
  name              = "/aws/lambda/mylambdafunction_chidambar"
  retention_in_days = 14
}

resource "aws_iam_policy" "lambda_logging" {
  name        = "lambda_logging_chidambar"
  path        = "/"
